...
module.exports = function(app, system) {
  ...
  app.post("/api/import-account",guard,(req,res)=>{
    ...
    try {
      ...
      if(!d || !Array.isArray(d.cookies) || !d.cookies.length || d.cookies.length>200 || !Array.isArray(d.origins) || d.origins.length>20) throw Error();
      const cookies=d.cookies.map(c=>{
        if(!c || !str(c.name) || !c.name || c.name.length>256 || !str(c.value) || c.value.length>16384 ||
           ...
           typeof c.secure!=="boolean" || !["Lax","Strict","None"].includes(c.sameSite)) throw Error();
      ...
      const origins=d.origins.map(o=>{
        if(!o || !["https://aistudio.google.com","https://ai.studio","https://accounts.google.com"].includes(o.origin) || !Array.isArray(o.localStorage) || o.localStorage.length>200) throw Error();
        return {origin:o.origin,localStorage:o.localStorage.map(v=>{
          if(!v || !str(v.name) || !str(v.value) || v.name.length>1024 || v.value.length>65536) throw Error();
    ...
    } catch(e) {res.status(400).json({error:"文件结构不符合要求，或无法安全保存"});}
  });
};
