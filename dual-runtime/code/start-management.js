...
const row=(slot,st)=>{const ok=st.ready&&st.active===0;const win=st.windowEnd&&st.windowEnd>Date.now()?'  窗口重置 '+new Date(st.windowEnd).toLocaleString('zh-CN',{hour12:false}):'  窗口未启用';return '<span class="label">实例 '+slot+' (账号 #'+(st.account??'-')+')</span>: <span class="'+(ok?"status-ok":"status-error")+'">'+(st.ready?((st.active||0)+' 个请求处理中'):(st.rotationBlocked?'轮换受阻，需人工恢复':'未就绪'))+'</span>  3.7flash '+(st.usesFlash37??0)+' / 100  3.8flash '+(st.usesFlash38??0)+' / 100  Pro '+(st.usesPro??0)+' / 10'+win+'\\n';};
