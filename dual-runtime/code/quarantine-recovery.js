...
class QuarantineRecovery {
 ...
 async check(slot){
  if(this.checking.has(slot)||this.dispatch.halted||this.occupied(slot)||this.rotation.running.has(slot)||this.rotation.failures.has(slot))return;
  ...
  try{
   ...
   if(!probe.quarantined||probe.busy||probe.activeRequests!==0)return;
   if(this.dispatch.halted||this.scheduler.closed||this.occupied(slot)||this.rotation.running.has(slot)||owner.pending||owner.current!==account)return;
   const now=Date.now();
