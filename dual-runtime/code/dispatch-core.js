...
class DispatchCore {
 constructor(pool,persist){
  ...
  this.slots=new Map(['A','B'].map(slot=>[slot,{active:0,requests:new Set(),usesFlash37:0,usesFlash38:0,usesPro:0,ready:false}]));
 ...
 commitRotation(ticket,oldClosed){
  ...
  const s=this.slots.get(ticket.slot);s.usesFlash37=0;s.usesFlash38=0;s.usesPro=0;s.ready=false;
  this.checkpoint();
 }
}
