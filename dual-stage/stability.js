...
class SerialGate {
 constructor(){this.busy=false;this.queue=[];this.context=new AsyncLocalStorage();}
 run(fn,valid=()=>true){
 ...
 async pump(){
  ...
  clearTimeout(item.timer);this.busy=true;
...
function install(h){
 ...
 h.logger.info("[Stability] 并发1，排队上限10，等待120秒；保留80次正常轮换");
}
